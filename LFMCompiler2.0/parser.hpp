...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // lambda_body
      char dummy6[sizeof (LambdaBody)];

      // letexpr
      char dummy7[sizeof (LetExprAST*)];
      ...
      // pair
      char dummy10[sizeof (std::pair<ExprAST*, ExprAST*>)];

      // binding
      // capture
      char dummy11[sizeof (std::pair<std::string, ExprAST*>)];

      // "id"
      char dummy12[sizeof (std::string)];

      // deflist
      char dummy13[sizeof (std::vector<DefAST*>)];
      ...
      // pairs
      char dummy15[sizeof (std::vector<std::pair<ExprAST*, ExprAST*>>)];
    ...
    TOK_TRUE = 278,                // "true"
    TOK_FALSE = 279,               // "false"
    TOK_EXTERN = 280,              // "external"
    TOK_DEF = 281,                 // "function"
    TOK_FORWARD = 282,             // "forward"
    TOK_LAMBDA = 283,              // "lambda"
    TOK_IF = 284,                  // "if"
    TOK_AND = 285,                 // "and"
    TOK_OR = 286,                  // "or"
    TOK_NOT = 287,                 // "not"
    ...
    TOK_UMINUS = 293,              // UMINUS
    TOK_NEGATE = 294               // NEGATE
      };
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_TRUE = 23,                             // "true"
        S_FALSE = 24,                            // "false"
        S_EXTERN = 25,                           // "external"
        S_DEF = 26,                              // "function"
        S_FORWARD = 27,                          // "forward"
        S_LAMBDA = 28,                           // "lambda"
        S_IF = 29,                               // "if"
        S_AND = 30,                              // "and"
        S_OR = 31,                               // "or"
        ...
        S_startsymb = 41,                        // startsymb
        S_deflist = 42,                          // deflist
        S_def = 43,                              // def
        S_extdef = 44,                           // extdef
        S_funcdef = 45,                          // funcdef
        S_forwarddef = 46,                       // forwarddef
        S_prototype = 47,                        // prototype
        S_params = 48,                           // params
        S_expr = 49,                             // expr
        S_lambda_body = 50,                      // lambda_body
        S_arglist = 51,                          // arglist
        S_args = 52,                             // args
        S_condexpr = 53,                         // condexpr
        S_pairs = 54,                            // pairs
        S_pair = 55,                             // pair
        S_boolexpr = 56,                         // boolexpr
        S_literal = 57,                          // literal
        S_relexpr = 58,                          // relexpr
        S_letexpr = 59,                          // letexpr
        S_bindings = 60,                         // bindings
        S_binding = 61,                          // binding
        S_captures = 62,                         // captures
        S_capture = 63                           // capture
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_lambda_body: // lambda_body
        value.move< LambdaBody > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, LambdaBody&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const LambdaBody& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_lambda_body: // lambda_body
        value.template destroy< LambdaBody > ();
        break;
    ...
    // Implementation of make_symbol for each token kind.
      ...
      {
        return symbol_type (token::TOK_LAMBDA, std::move (l));
      }
#else
      static
      symbol_type
      ...
      {
        return symbol_type (token::TOK_LAMBDA, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_lambda_body: // lambda_body
        value.copy< LambdaBody > (YY_MOVE (that.value));
        break;
  ...
  parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_lambda_body: // lambda_body
        value.move< LambdaBody > (YY_MOVE (s.value));
        break;
