...
/// BodyExprAST - Classe per la rappresentazione del body di una funzione
class BodyExprAST : public ExprAST {
private:
   std::vector<std::pair<std::string, ExprAST*>> Assignments;
   ExprAST* value;

public:
   BodyExprAST(std::vector<std::pair<std::string, ExprAST*>> Assignments, ExprAST* value);
   void visit() override;
   Value *codegen(driver& drv) override;
};
...
/// GlobalVariableAST - Classe che rappresenta la definizione di una variabile globale
class GlobalVariableAST : public DefAST {
private:
  std::string Name;
...
public:
  ...
  GlobalVariableAST(std::string Name, ExprAST* Val);
  Value *codegen(driver& drv) override;
  void visit() override;
};
