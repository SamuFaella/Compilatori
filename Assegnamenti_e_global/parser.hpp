...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    {
      // body
      char dummy1[sizeof (BodyExprAST*)];

      // def
      char dummy2[sizeof (DefAST*)];
      ...
      // globalvar
      char dummy5[sizeof (GlobalVariableAST*)];
    ...
    TOK_IN = 285,                  // "in"
    TOK_END = 286,                 // "end"
    TOK_GLOBAL = 287,              // "global"
    TOK_RETURN = 288,              // "return"
    TOK_IDENTIFIER = 289,          // "id"
    TOK_NUMBER = 290,              // "number"
    TOK_UMINUS = 291,              // UMINUS
    TOK_NEGATE = 292               // NEGATE
      };
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_body = 46,                             // body
        S_assignments = 47,                      // assignments
        S_prototype = 48,                        // prototype
        S_params = 49,                           // params
        S_expr = 50,                             // expr
        ...
        switch (this->kind ())
    {
      case symbol_kind::S_body: // body
        value.move< BodyExprAST* > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, BodyExprAST*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const BodyExprAST*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      case symbol_kind::S_body: // body
        value.template destroy< BodyExprAST* > ();
        break;
    ...
    , location (that.location)
  {
    ...
    {
      case symbol_kind::S_body: // body
        value.copy< BodyExprAST* > (YY_MOVE (that.value));
        break;
  ...
  parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    {
      case symbol_kind::S_body: // body
        value.move< BodyExprAST* > (YY_MOVE (s.value));
        break;
